...
- (id)initWithMessageParameters:(UInt8)opcode andSubOpcode:(UInt8)subOpcode andData:(NSData *)data; {
    ...
    UInt8 crc = [FlomioMessage calculateCRCForIncompleteMessage:message];
...
- (id)initWithData:(NSData *)theData; {
    ...
    if (self) {
        if (theData != nil) {
            ...
            _subOpcode = [FlomioMessage getMessageSubOpcode:theData];
...
+ (UInt8)calculateCRCForIncompleteMessage:(NSData *)theData
{
    ...
    return [FlomioMessage calculateCRCForIncompleteMessage:bytes withLength:theData.length];
}
